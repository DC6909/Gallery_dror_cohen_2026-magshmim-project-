...
{
	size_t space_pos = album.getCreationDate().find(' ');
	std::string time_only = album.getCreationDate().substr(space_pos + 1);
	strOut << "[" << album.m_name << "] - created by user@"
		<< const_cast<Album&>(album).getOwnerId() << " - creation time " << time_only <<std::endl;
