...
HANDLE AlbumManager::_global_hAppPic = NULL;
AlbumManager::AlbumManager(IDataAccess& dataAccess) :
    m_dataAccess(dataAccess), m_nextPictureId(0), m_nextUserId(0)
{
	// Left empty
	m_dataAccess.open();
	m_nextPictureId = m_dataAccess.getLastPictureId() + 1;
	m_nextUserId = m_dataAccess.getLastUserId() + 1;
}
...
BOOL WINAPI AlbumManager::consoleHandler(DWORD signal)
{
	...
	{
		if (_global_hAppPic != NULL)
		{
			TerminateProcess(_global_hAppPic, 0);// kill the process that we create(the image app)...
			
			// need to clean the henlder
			CloseHandle(_global_hAppPic);
			_global_hAppPic = NULL;
			return true;
		}
...
{
	//fucniton taht tacke path to the app.exe file and the path to the pictrue

	STARTUPINFO si;
	PROCESS_INFORMATION pi;

	ZeroMemory(&si, sizeof(si)); // to initiation to zero the data structers
	si.cb = sizeof(si);
	ZeroMemory(&pi, sizeof(pi));

	std::string commandLine = "\"" + appPath + "\" \"" + picPath + "\"";

	//we need to create a copy of the commendLine beacuse this a const char* and the fucnitoin need to a regaluer cahr*
	char* copyCmd = _strdup(commandLine.c_str());
...
void AlbumManager::showPicture()
{
	...
	std::cout << "1.paint app\n";
	std::cout << "2.IrfraView app\n";
	std::string appN = getInputFromConsole("Enter app num: ");
	if (appN == "1")
	{
	...
	{
		openPicInSpecificApp("C:\\Program Files\\IrfanView\\i_view64.exe", pic.getPath());
	}
	else
	{
		std::cout << "not option: " << appN << std::endl;
	}
}
...
void AlbumManager::removeUser()
{
	...
	m_dataAccess.unTagUserFromAllPicture(user); /*reemovs all the tagss*/
	m_dataAccess.deleteAllbumsUser(user);//remove all the allbums
