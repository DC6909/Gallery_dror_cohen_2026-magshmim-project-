...
void MemoryAccess::deleteAllbumsUser(const User& user)
{
	auto it = m_albums.begin();
	while (it != m_albums.end()) {
...
void MemoryAccess::unTagUserFromAllPicture(const User& user)
{
	for (auto& album : m_albums) {
